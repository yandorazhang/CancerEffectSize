...
filename = (paste0("../../data_new/",trait_name,"/summarydata_RemoveOutlier.RData") )
if( file.exists(filename)) {
  ...
  fit = genesis(summarydata, filter=FALSE, 
                ...
                qqplot=TRUE, qqplotCI.coverage=0.8, qqplot.name=paste0(output_path,"/fit2_RemoveOutlier_pic",startingpic), 
                summaryGWASdata.save=FALSE, qqplotdata.save=T, 
                herit.liability=FALSE)
  
  save(fit, file=paste0(output_path,"/fit2_RemoveOutlier_pic",startingpic,".RData"))
  ...
  for(startingpic in c(0.1, 0.05, 0.01, 0.005, 0.001, 0.0005, 0.0001)){
    load(paste0(output_path,"/fit2_RemoveOutlier_pic",startingpic,".RData"))
  ...
  load(paste0(output_path,"/fit2_RemoveOutlier_pic",startingpic,".RData"))
  save(fit,bestpic, file=paste0(output_path,"/bestfit2_RemoveOutlier.RData"))
  ...
  fit = genesis(summarydata, filter=FALSE, 
                ...
                qqplot=TRUE, qqplotCI.coverage=0.8, qqplot.name=paste0(output_path,"/fit3_RemoveOutlier"), 
                summaryGWASdata.save=FALSE, qqplotdata.save=T, 
                herit.liability=FALSE)
  
  save(fit, file=paste0(output_path,"/fit3_RemoveOutlier.RData"))
}
