...
for(iter in inx){
  ...
  if(!file.exists(filename)) load(paste0("../../data_new/",trait_name,"/summarydata.RData") )
  ...
  tem[i,] = c(trait_name_plot,ncase, ncontrol,
              f(as.numeric(ncase),as.numeric(ncontrol)),current.case, current.control, nrow(summarydata))
  i = i+1
}
