...
for(i in 1:14){
  ...
  load(paste0("../../result_png_csv_new_clump/RData/","Fig34",trait_name,".RData"))
  lp[[i]] = p_p
  lauc[[i]] = p_auc
  lpheno[[i]] = p_pheno
  lr99[[i]] = p_r99
...
#---------------
pdf(paste0("../../result_png_csv_new_clump//Fig4.pdf"), width=25, height=25)
...
# #---------------
# setEPS()
# postscript(paste0("../../result_png_csv_new_clump//Fig4.eps"), width=25, height=25)
# # do.call(grid.arrange,c(lr99,ncol=4,as.table=T))
# grid.arrange(
#   arrangeGrob(grobs=lapply(lr99,  function(p) p ), ncol=4,
#               bottom=textGrob("x: Total sample size assuming 1:1 case:control ratio (in thousands)", vjust=-2,gp=gpar(fontsize=25)),
